...
create = "{ SizedCache::with_size(authz_cache_size()) }",
...
pub fn clear_authz_cache() -> Result<(), String> {
    let mut cache = crate::caching::CHECK_AUTHORIZATION
        .lock()
...
pub fn authz_cache_stats() -> Result<AuthzCacheStats, String> {
    let cache = crate::caching::CHECK_AUTHORIZATION
        .lock()
        .map_err(|e| format!("Failed to lock cache: {}", e))?;
